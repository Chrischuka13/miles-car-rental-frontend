...
import { adminCreateBookingApi } from "@/api/admin";
...
interface NewBookingModalProps {
  isOpen: boolean;
  onClose: () => void;
  cars: { _id: string; brand: string; modelName: string; images: string[] }[];
}
...
export default function NewBookingModal({
  ...
  const {
    ...
    resolver: zodResolver(validateAdminNewBookingSchema),
    defaultValues: {
      driverOption: false,
  ...
  //     pickupTime: data.pickupTime || "09:00 AM",
  //     returnTime: data.returnTime || "09:00 AM",
  //   };
  ...
  const handleContinue = async () => {
    if (step === 1) {
      ...
      if (!watchedValues.pickupDate) {
        setError("pickupDate", { type: "manual", message: "Pickup date is required" });
        absoluteValid = false;
      } else {
      ...
      if (!watchedValues.returnDate) {
        setError("returnDate", { type: "manual", message: "Return date is required" });
        absoluteValid = false;
      } else {
        ...
        <form onSubmit={handleSubmit(onSubmitForm)} noValidate>
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <div>
                ...
                {errors.car && (
                  <p className="text-red-500 text-xs mt-1">{errors.car.message}</p>
                )}
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  {errors.returnLocation && (
                    <p className="text-red-500 text-xs mt-1">{errors.returnLocation.message}</p>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center justify-between py-1">
                ...
                >
                  <span className={`absolute top-1 left-0 w-4 h-4 bg-white rounded-full shadow transition-transform duration-200 ${withDriver ? "translate-x-7" : "translate-x-1"}`} />
                </button>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full border border-gray-200 rounded-full px-4 py-3 text-sm outline-none focus:border-[#F97316] appearance-none bg-white"
                  >
                    <option value="Pay_with_Bank_Transfer">Bank Transfer</option>
                  </select>
                  <ChevronDown size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                </div>
              </div>
          ...
          {step === 3 && (
            <div className="space-y-3 text-sm">
              <p className="text-xs font-bold uppercase tracking-widest text-[#F97316]">Customer</p>
              <div className="space-y-2 bg-[#F9F7F4] p-4 rounded-lg">
                <div className="flex justify-between">
                  <span className="text-gray-500">Name</span>
                  <span className="font-medium text-gray-900">{watchedValues.fullname || "—"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Email Address</span>
                  <span className="font-medium text-gray-900">{watchedValues.email || "—"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Phone Number</span>
                  <span className="font-medium text-gray-900">{watchedValues.phone || "—"}</span>
                </div>
              </div>

              <hr className="border-gray-100 my-2" />

              <p className="text-xs font-bold uppercase tracking-widest text-[#F97316]">Vehicle</p>
              <div className="space-y-2 bg-[#F9F7F4] p-4 rounded-lg">
                <div className="flex justify-between">
                  <span className="text-gray-500">Vehicle</span>
                  <span className="font-medium text-gray-900">
                    {cars.find((c) => c._id === watchedValues.car)
                      ? `${cars.find((c) => c._id === watchedValues.car)?.brand} ${cars.find((c) => c._id === watchedValues.car)?.modelName}`
                      : "—"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Pick up Location</span>
                  <span className="font-medium text-gray-900">{watchedValues.pickupLocation || "—"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Return Location</span>
                  <span className="font-medium text-gray-900">{watchedValues.returnLocation || "—"}</span>
                </div>
                <div className="flex justify-between">
              ...
              <p className="text-xs font-bold uppercase tracking-widest text-[#F97316]">Price Breakdown</p>
              <div className="space-y-2 bg-[#F9F7F4] p-4 rounded-lg">
                <div className="flex justify-between">
                  <span className="text-gray-500">{dynamicDays} days × {formatNaira(basePricePerDay)}</span>
                  <span className="font-medium text-gray-900">{formatNaira(computedVehicleTotal)}</span>
                </div>
                {withDriver && (
                  <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-gray-900">{formatNaira(serviceFeeConstant)}</span>
                </div>
                <div className="flex justify-between pt-3 border-t border-gray-100">
                  <span className="font-bold text-gray-900">Total</span>
                  <span className="font-bold text-gray-900 text-base">{formatNaira(finalTotalPrice)}</span>
                </div>
              </div>
            </div>
          )}
